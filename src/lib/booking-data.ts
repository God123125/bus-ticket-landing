...
export function searchTrips(
  from?: string | Geographic,
  to?: string | Geographic,
) {
  const fromStr =
    typeof from === "object"
      ? from?.name_en || from?.name_kh || from?._id
      : from;
  const toStr =
    typeof to === "object" ? to?.name_en || to?.name_kh || to?._id : to;
  return TRIPS.filter(
    (t) =>
      (!fromStr ||
        t.from.name_en === fromStr ||
        t.from.name_kh === fromStr ||
        t.from._id === fromStr) &&
      (!toStr ||
        t.to.name_en === toStr ||
        t.to.name_kh === toStr ||
